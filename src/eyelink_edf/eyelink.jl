...
Extract saccade events from the EDF data.
"""
saccades(edf::EDFFile) = parse_saccades(edf.events)

"""
    fixations(edf::EDFFile) -> DataFrame

Extract fixation events from the EDF data.
"""
fixations(edf::EDFFile) = parse_fixations(edf.events)

"""
    blinks(edf::EDFFile) -> DataFrame

Extract blink events from the EDF data.
"""
blinks(edf::EDFFile) = parse_blinks(edf.events; samples = edf.samples)

"""
    aois(edf::EDFFile) -> DataFrame

Extract Area of Interest (AOI) definitions from the EDF data.
"""
aois(edf::EDFFile) = parse_aois(edf.events)

"""
    messages(edf::EDFFile; summary=false) -> DataFrame

Extract message events from the EDF data.
...
# Example
```julia
messages(edf)               # all individual messages
messages(edf; summary=true) # frequency table
```
"""
function messages(edf::EDFFile; summary::Bool = false)
    msg = parse_messages(edf.events)
    !summary && return msg
    ...
    for m in msg.message
        s = strip(string(m))
        isempty(s) && continue
        msg_counts[s] = get(msg_counts, s, 0) + 1
    end
