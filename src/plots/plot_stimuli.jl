...
function plot_stimuli(stim::Dict; audio_device = nothing, resolution = (1920, 1080))
    ...
    # Controls
    play_button =
        Button(f[3, 1:2], label = "", buttoncolor = :transparent, tellwidth = false)
    ...
    sl = Slider(
        nav_grid[1, 2],
        range = lift(x -> 1:max(1, length(x)), filtered_keys),
        startvalue = 1,
    )
    next_button = Button(nav_grid[1, 3], label = "Next >", width = 80)

    counter_label =
        Label(f[5, 1:2], "File 1 of $(length(keys_list))", fontsize = 16, tellwidth = false)
