...
function saccade_metrics(
        ...
        while i <= n
            if in_sacc[i]
                ...
                if t_start <= sacc_t <= t_end
                    ...
                    valid_coords = filter(k -> !isnan(gx[k]) && !isnan(gy[k]), collect(run))
                    ...
                    if length(valid_coords) >= 2
                        ...
                        if amplitude_px > 1e-6
                            for k in valid_coords
                                xi, yi = gx[k], gy[k]
                                dist = abs(dx * (ys - yi) - (xs - xi) * dy) / amplitude_px
                        ...
                        curvature_ratio = amplitude_px > 1e-6 ? max_curvature_px / amplitude_px : 0.0
    ...
    # Empty safety guard
    if nrow(res) == 0
        expected_cols = vcat(group_cols, [:saccade_idx, :duration, :start_x, :start_y, :end_x, :end_y, :amplitude_deg, :peak_velocity, :max_curvature_px, :curvature_ratio])
